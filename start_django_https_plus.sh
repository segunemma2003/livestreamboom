...
echo "🐍 Starting Django Backend with HTTPS (runserver_plus)"
...
# Use runserver_plus with SSL (port 8001 for HTTPS)
...
if lsof -Pi :8001 -sTCP:LISTEN -t >/dev/null 2>&1; then
    ...
    echo "  • Django HTTPS running on port 8001 (regular HTTP still on 8000)"
    ...
    echo -e "${BLUE}🛑 To stop Django HTTPS server:${NC}"
    echo "  kill $DJANGO_PID"
    echo "  or: pkill -f runserver_plus"
else
